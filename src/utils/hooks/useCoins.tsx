    ...
    CoinsAPI.GetCoins(filter)
      .then((data) => {
        switch (filter.order_by) {
          case "":
            setCoins(
              data.data.map(el=>ApiValuesTransformer.fiterZeroValues(el)).map(el=>ApiValuesTransformer.formatCost(el))
            );
            break;
          case "pricedesc":
            setCoins(
              (
                data.data.map(el=>ApiValuesTransformer.fiterZeroValues(el))
              ).sort(
                (a: Assets, b: Assets) =>
          ...
          case "priceasc":
            setCoins(
              (
                data.data.map(el=>ApiValuesTransformer.fiterZeroValues(el)).sort(
                (a: Assets, b: Assets) =>
