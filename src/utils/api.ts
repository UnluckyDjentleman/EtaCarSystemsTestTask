...
export default class CoinsAPI {
  public static async GetCoins(filter: Filter): Promise<DataAssetsArray> {
    const resp = await axios.get(
      import.meta.env.VITE_API_COINS_URL + "/assets",
      {
        params: {
          limit: filter.limit,
  ...
  public static async GetCoinById(id: string): Promise<DataAssets> {
    const resp = await axios.get(
      import.meta.env.VITE_API_COINS_URL + "/assets/" + id,
    );
    return resp.data;
  }
  ...
  public static async GetCoinHistoryById(
    id: string,
    filterHistory: FilterHistory,
  ): Promise<DataAssetsHistoryArray> {
    const resp = await axios.get(
      import.meta.env.VITE_API_COINS_URL + "/assets/" + id + "/history",
      {
        params: {
